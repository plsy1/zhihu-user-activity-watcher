# Zhihu User Activity Watcher

一个用于采集知乎用户动态时间线，并导出给 AI 分析用户画像、兴趣和行为习惯的 Tampermonkey 脚本。

## 安装

[![安装 userscript](https://img.shields.io/badge/安装-userscript-2ea44f?style=for-the-badge)](https://raw.githubusercontent.com/plsy1/zhihu-user-activity-watcher/main/zhihu-activity-watcher.user.js)

先安装 Tampermonkey，然后点击上面的安装按钮。正常情况下，Tampermonkey 会自动打开脚本安装页面。

脚本已经配置 `@updateURL` 和 `@downloadURL`。后续只要仓库 `main` 分支里的脚本版本号更新，Tampermonkey 就可以通过正常更新检查获取新版。

## 使用

1. 在浏览器里安装 Tampermonkey。
2. 点击上方安装按钮安装脚本。
3. 打开知乎用户主页，例如 `https://www.zhihu.com/people/<token>/activities`。
4. 使用页面右下角的浮动面板：
   - `动态页`：从用户主页跳转到动态页。
   - `API直采` / `DOM滚动`：切换采集模式；只切换模式，不会立即开始采集。
   - `开始`：按当前选择的模式开始采集。
   - `暂停`：停止当前采集任务。
   - `JSON`：导出完整结构化数据。
   - `CSV`：导出适合表格查看的数据。
   - `AI+摘要`：导出 Markdown，包含 AI 分析 prompt、精简 timeline 和每条动态摘要。
   - `AI精简`：导出 Markdown，包含 AI 分析 prompt 和精简 timeline，不含摘要，更省 token。
   - `清空`：清空当前浏览器本地保存的采集结果。
   - `清理页面`：手动移除页面上较早的动态 DOM 卡片，只保留最近一批，缓解长时间滚动后的卡顿。
   - `间隔`：设置最小/最大间隔。采集时每次请求或滚动都会在这个范围内随机等待。

面板状态会显示当前已采集条数、时间范围、API 页数和续采点状态，方便判断已经采到哪一天。

## 数据字段

每条动态会被整理成一条 timeline 记录：

- `actionType` / `actionLabel`：动作类型，例如点赞/赞同、关注、回答、收藏、文章等。
- `targetType`：目标类型，例如回答、问题、文章、用户、专栏、收藏夹等。
- `targetTitle`：动态目标标题或可见文本。
- `targetUrl`：动态目标链接。
- `timeText`：知乎页面显示的原始时间文本。
- `timeIso`：尽力解析得到的结构化时间；解析不确定时可能为空。
- `source`：数据来源，`api` 表示来自知乎页面自身的动态接口响应，`dom` 表示来自页面可见 DOM 解析。
- `summary`：动态卡片里的可见摘要文本。

## AI 导出

当你想把结果交给大模型分析时，使用 `AI+摘要` 或 `AI精简`。

AI Markdown 会包含：

- 分析任务 prompt。
- 元数据，例如用户 token、导出时间、动态条数、时间范围。
- 按时间排列的 timeline。
- 可选摘要：`AI+摘要` 包含摘要，`AI精简` 不包含摘要。

AI prompt 会要求模型分析：

- 用户整体画像。
- 兴趣偏好和高频主题。
- 内容偏好。
- 点赞、关注、回答、收藏等行为特征。
- 活跃日期、活跃小时段和集中活跃窗口。
- 可能的职业、学习方向、技术/专业背景。
- 用户标签。
- 明确事实、合理推测、弱信号和无法判断内容。

AI Markdown 会省略链接和原始 JSON，以减少 token。需要完整 `targetUrl` 或结构化字段时，请使用 JSON/CSV 导出。

## 采集方式

脚本不会绕过登录、验证码、风控、权限或访问控制。它只读取当前浏览器里当前账号可见的知乎动态。

脚本支持三种采集路径：

- DOM 滚动采集：选择 `DOM滚动` 后点击 `开始`，脚本会慢速下滑页面，从已经渲染出来的动态卡片中提取数据。
- API 响应监听：监听页面自己正常发出的动态接口响应，例如 `/api/v3/moments/<user>/activities`，从响应里补充结构化数据。
- API 直采：选择 `API直采` 后点击 `开始`，脚本会在当前登录页面中使用同源请求访问动态接口，并跟随接口返回的分页信息继续采集。成功时可以避免长时间滚动造成的页面渲染卡顿。

采集间隔是随机范围，例如设置为 `500 - 1500 ms` 时，每次 API 请求或 DOM 滚动前都会随机等待 500 到 1500 毫秒。旧版单个间隔设置会自动迁移成最小值和最大值相同的范围。

API 直采会按用户 token 保存续采位置。每成功拿到一页，就把下一页地址保存到本地；如果中途失败或刷新页面，下次再点击 `开始` 会从上次成功页之后继续。面板会显示 `续采点已存` 或 `无续采点`。

脚本不会生成或破解知乎签名。如果知乎接口要求签名、验证码或更严格的校验，`API直采` 会失败并在面板显示错误；此时仍可使用 DOM 滚动采集。

这些方式有一些限制：

- 只能采集当前账号能看到的内容。
- 依赖知乎前端 DOM 结构，知乎改版后可能需要调整选择器。
- API 响应结构也可能随知乎改版变化，接口监听只是增强能力，不是唯一采集路径。
- API 直采能否成功取决于知乎当前接口策略；地址栏能打开不代表脚本 `fetch` 在所有情况下都能成功。
- 如果页面懒加载很慢，滚动太快可能漏掉部分内容。
- 不适合大规模爬取。

## 本地保存

采集结果会保存在当前浏览器的 `localStorage` 中：

```text
zhihu-activity-watcher.items
zhihu-activity-watcher.api-cursors
```

刷新页面后数据和 API 续采位置仍会保留。再次采集时，脚本会根据动作类型、链接和时间去重，避免重复累计。点击 `清空` 会删除这些本地数据和当前用户的 API 续采位置。

## 注意事项

请仅用于个人归档、公开信息整理或合规研究。使用时应尊重知乎服务条款、访问频率限制和用户隐私。
