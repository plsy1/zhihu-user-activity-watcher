...
4. 使用页面右下角的浮动面板：
   ...
   - `开始`：慢速下滑并采集当前账号可见的动态。
   - `暂停`：停止下滑。
   - `API直采`：尝试使用当前登录浏览器环境直接请求知乎动态接口分页采集，成功时不需要滚动渲染页面。
   - `JSON`：导出完整结构化数据。
   - `CSV`：导出适合表格查看的数据。
...
- DOM 滚动采集：点击 `开始` 后慢速下滑页面，从已经渲染出来的动态卡片中提取数据。
- API 响应监听：监听页面自己正常发出的动态接口响应，例如 `/api/v3/moments/<user>/activities`，从响应里补充结构化数据。
- API 直采：点击 `API直采` 后，脚本会在当前登录页面中使用同源请求访问动态接口，并跟随接口返回的分页信息继续采集。成功时可以避免长时间滚动造成的页面渲染卡顿。
