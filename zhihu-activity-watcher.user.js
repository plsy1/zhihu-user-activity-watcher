...
(function () {
  ...
  function buildAnalysisPrompt() {
    return [
      "你是一名谨慎的开源信息分析助手。下面是一位知乎用户动态页中可见的 timeline 数据，数据可能不完整，时间字段可能是页面显示的相对时间。",
      "",
      "请基于数据做分析，但不要把推测当事实。请输出：",
      "1. 按时间顺序概括该用户近期关注/点赞/回答/收藏的主题变化。",
      "2. 归纳反复出现的话题、领域、观点倾向或兴趣簇，并说明对应证据。",
      "3. 标出异常密集的活动时间段或行为模式。",
      "4. 区分明确事实、弱信号和不可判断内容。",
      "5. 给出后续人工核查建议，例如哪些链接值得打开确认。",
      "",
      "隐私与合规要求：只分析数据中出现的公开可见动态，不推断敏感身份、住址、联系方式、健康、宗教、政治派别等高度敏感个人属性。",
    ].join("\n");
  }
  ...
  function buildPromptMarkdown() {
    ...
    const timelineLines = state.items.map((item, index) => {
      ...
      return `${index + 1}. [${time}] ${item.actionLabel} ${target}${url}${summary}`;
    });
